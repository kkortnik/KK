...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentView }) => {
  ...
  const handleNav = (view: 'home' | 'portfolio') => {
    onNavigate(view);
    setIsOpen(false);
    ...
    <nav className="fixed top-0 left-0 w-full bg-white/95 backdrop-blur-sm z-50 py-4 transition-all duration-300 border-b border-gray-100/50">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex items-center justify-between h-20">
        ...
        <div className="hidden md:flex gap-8 text-xs font-bold tracking-widest uppercase text-black">
          <button 
            ...
            className={`hover:text-brand-red transition-colors ${currentView === 'home' ? 'text-brand-red' : ''}`}
          >
            Home
          </button>
          <button 
            onClick={() => handleNav('portfolio')} 
            className={`hover:text-brand-red transition-colors ${currentView === 'portfolio' ? 'text-brand-red' : ''}`}
          >
            Work
          </button>
        </div>
        ...
        {/* Center: Brand Logo */}
        <div className="absolute left-1/2 transform -translate-x-1/2 flex flex-col items-center group cursor-pointer" onClick={() => handleNav('home')}>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 absolute w-full left-0 top-full flex flex-col items-center py-10 gap-6 animate-in fade-in slide-in-from-top-5 shadow-xl h-screen">
          <button onClick={() => handleNav('home')} className="text-sm font-bold uppercase tracking-widest">Home</button>
          <button onClick={() => handleNav('portfolio')} className="text-sm font-bold uppercase tracking-widest">Work</button>
